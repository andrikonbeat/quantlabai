...
- [x] 5.1 Flow-integrity assertion: after full agent run, `assert_flow(PHASES)` passes; `PHASES` still 14; no `campaign/flow.py` edit (REQ-37) — evidence: live `len(PHASES) == 14` / `len(STAGE_FOR_PHASE) == 14`; `git diff d0a6b16..HEAD -- sdk/quantlab/campaign/flow.py` EMPTY (exit 0); `TestFlowIntegrityAfterRun::test_run_leaves_phases_at_14_and_registry_unchanged` + `TestAgentRunFlowIntegrity::test_agent_run_keeps_14_phases_and_mapping` passed (verify phase)
- [x] 5.2 Full suite: `SQX_FORCE_MOCK=1 PYTHONPATH=sdk python3 -m pytest -q sdk/tests/test_guardian/ sdk/tests/test_pipeline_agent_stages.py tests/guardian/ tests/pipeline/test_live_ops_stage.py tests/campaign/test_flow_segments.py --tb=short` — evidence: declared suite ran (see verify-report.md) → 89 passed, 1 pre-existing failure (stale `AI/opencode/agents/campaign.md` assertion, proven pre-existing at d0a6b16; NOT attributed to this change); config parse exit 0 (verify phase)
- [x] 5.3 Validate `opencode.json` parses; confirm `guardian.md`/`orchestrator.md`/`campaign.md` unchanged except intended rows — evidence: `python3 -m json.tool` exit 0; `quantlab-guardian` registered deny-first with explicit orchestrator allow; `orchestrator.md` GUARDIAN route + non-guardian MUST NOT; `campaign.md` phase 14 delegates; `guardian.md` present (2.8K) (verify phase)
