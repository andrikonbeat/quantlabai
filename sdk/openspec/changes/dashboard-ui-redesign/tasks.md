...
- [x] PR6-01: WCAG 2.1 AA audit all 4 templates — ARIA labels, sort headers, aria-live on log, search inputs, metric card labels
- [x] PR6-02: Add `@media (prefers-reduced-motion: reduce)` — global block in base.css, spinning/stage-pulse in utilities.css, sidebar overlay in dashboard-grid.css, `--duration-instant` token
- [x] PR6-03: Delete `dashboard.css` and `dashboard.js`, remove `DASHBOARD_V2` feature flag, keep only modular CSS/JS
- [x] PR6-04: Performance verification — document JS/CSS sizes, Plotly loading strategy, Lighthouse expectations
